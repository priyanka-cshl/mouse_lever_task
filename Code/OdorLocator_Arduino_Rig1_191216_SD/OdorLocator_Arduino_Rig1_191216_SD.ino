...
void setup()
{
  ...
  openlooptrialstates.UpdateOpenLoopTrialParams(PID_mode_trial_timing); // 13.02.18
}
...
void loop()
{
  ...
  else if (replay_mode) // 19.12.16
  {
    SPIWriter(dac_spi_pin, lever_position);
  }
  else if (PID_mode) // 13.02.18
  {
    motor_location = map(PID_mode_location, 0, 255, 0, 65534); // max location is 256 - 8 bit
    SPIWriter(dac_spi_pin, lever_position);
  }
  else // not used
  {
    ...
    if (timer_override)
    {
      switch (trialstate[1])
      {
        case 0: // move motor to desired location and give it time to settle
          stimulus_state[1] = PID_mode_location;
      ...
      case 10: // opening/closing handshake
        switch (FSMheader - 10)
        {
          ...
          case 1: // Acquisition start handshake
            ...
            //reset flags for open loop and replay
            openloop_state = 0;
            openloop_flag = 0;
            current_reward_state = 0;
            current_odor_state = 0;
            current_air_state = 0;
            //other flags
      ...
      case 20: // update variables
        switch (FSMheader - 20)
        {
          ...
          case 1: // update variables for open loop mode - 23.01.2018
            ...
            UpdatePIDModeParams(); // parse param array to variable names and update motor params
            break;
        }
        break;
...
void UpdatePIDModeParams() // 23.01.2018
{
  myUSB.writeUint16(98);
  trialstate[0] = 5;
  // parse param array to variable names
  //which_odor = PID_param_array[0]; // odor vial number
  PID_mode_location = PID_param_array[1]; // desired location
  for (i = 0; i < 6; i++)
  {
    PID_mode_trial_timing[i] = PID_param_array[2 + i]; // motor_settle, pre-odor, odor, post-odor, iti in ms
  }
  ...
  // update trial state params
  openlooptrialstates.UpdateOpenLoopTrialParams(PID_mode_trial_timing);
}
...
void MoveMotor()
{
  ...
  if (!motor_override)// && (trialstate[1] == 4))
  {
    ...
    {
      if (openloop_flag == 4) // replay_mode
      {
        // read stim state
        if (mySDFile.available())
        {
          ...
          // deconstruct stimulus state, reward state and odor states
