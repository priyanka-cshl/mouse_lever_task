...
function [MyData, MyParams, TargetZones, FakeTargetZones] = ExtractTracesAndEvents(FileName)
...
for thisTrial = 1:size(MyParams,1)
    ...
    %% if point grey triggering was off - ignore triggers coming from the camera
    if MyParams(thisTrial,3)~=1
        MyData(f,16:17) = 0*MyData(f,16:17);
    end
end
