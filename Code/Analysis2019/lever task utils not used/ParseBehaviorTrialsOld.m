...
function [Traces, TrialInfo, TargetZones, TrialTimeStamps] = ...
    ParseBehaviorTrialsOld(MyData, MySettings, DataTags, sessionstart, sessionstop)
...
global MyFileName;
global OepsTrials; % Trial timestamps from OEPS - use to correct digital signal drift on rig-II
...
%     end
    ...
    % Odor ON timestamp
    LeverSnippet = MyData(LastTrialIdx:thisTrialIdx(1), LeverCol);
    ...
    TrialInfo.TrialStart(thisTrial,1) = (Initiations(OdorStart,2) + LastTrialIdx - start_idx)/SampleRate;
...
%                         end
    ...
    LastTrialIdx = thisTrialIdx(2); % current trial's end Idx
end
