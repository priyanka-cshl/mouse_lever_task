...
// PassiveTuning_related - 13.02.18
int PassiveTuning_mode = 0; // deliver odor stimuli at different locations
...
void loop()
{
  ...
  if (Session_Mode == 1 || Session_Mode == 2 || Session_Mode == 3 || Session_Mode == 4) // 1: closed-loop, 2: Replay, 3: Passive Tuning, 4: Passive Replay
  {
    SPIWriter(dac_spi_pin, lever_position);
  }
  //----------------------------------------------------------------------------

  if (Session_Mode == 0) // TF Calibration - currently unused
  {
  ...
  if (Session_Mode == 1) // Closed-loop
  {
    ...
    //----------------------------------------------------------------------------
    // 3) Is the lever currently in the Target Zone, given current stimulus state
    //----------------------------------------------------------------------------
    ...
    //----------------------------------------------------------------------------
    // 4) update stimulus state, direction etc. if the stimulus_state has changed
    //----------------------------------------------------------------------------
  ...
  if (Session_Mode == 3) // Passive Tuning
  {
    //----------------------------------------------------------------------------
    // 6) manage reporter pins, valves etc based on time elapsed since last event
    //----------------------------------------------------------------------------
  ...
  if (timer_override)
  {
    ...
    else if (Session_Mode == 3) // Passive Tuning
    {
  ...
  if ((trialstate[1] != trialstate[0]) && (Session_Mode == 1)) // trial state changes, closed-loop
  {
  ...
  if (trialstate[1] == 5 && trialstate[0] == 5 && odor_valve_state && (Session_Mode == 1) && (micros() - trial_timestamp) > 1000 * normal_iti)
  {
    //odor_valve_state = true;
    air_valve_state = false;
    send_odor_to_manifold();
  ...
  if ((trialstate[1] != trialstate[0]) && (Session_Mode == 3)) // trial state changes, passive tuning 
  {
    trial_timestamp = micros();
    if (timer_override)
    {
  ...
  if (replay_flag == 4) // Session_Mode == 2
  {
    // reward
