...
void loop()
{
  ...
  if (close_loop_mode)
  {
    digitalWrite(target_valves[0], (target_valve_state[0] || ((trialstate[0] > 0) && (trialstate[0] < 5)))); // open odor valve
    digitalWrite(target_valves[1], (target_valve_state[1] || ((trialstate[0] > 0) && (trialstate[0] < 5)))); // open air valve
  ...
  {
    digitalWrite(target_valves[0], target_valve_state[0]); // open odor valve
    digitalWrite(target_valves[1], target_valve_state[1]); // open air valve
    digitalWrite(trial_reporter_pin, ((trialstate[0] > 0) && (trialstate[0] < 5))); // active trial?
    //digitalWrite(in_target_zone_reporter_pin, in_target_zone[1]); // in_target_zone?
    digitalWrite(in_reward_zone_reporter_pin, target_valve_state[0]); // is Odor valve ON?
  }
  else if (!cleaningON)
  {
    digitalWrite(target_valves[0], (target_valve_state[0] || ((trialstate[0] > 0) && (trialstate[0] < 5)))); // open odor valve
    ...
    if (timer_override)
    {
      switch (trialstate[1])
      {
        case 0: // move motor to desired location and give it time to settle
          ...
          target_valve_state[0] = false; // keep odor valve closed (blank vial is Onand going to exhaust)
...
//          }
          for (i = 0; i < 4; i++)
          {
            digitalWrite(odor_valves[i], (i == open_loop_odor)); // turn on odor vial
          }
          target_valve_state[0] = true; // keep odor valve closed
...
//          }
          target_valve_state[0] = true; // turn odor valve ON
          target_valve_state[1] = true; 
          break;
        case 3: // post-odor, switch to air vial
          ...
          target_valve_state[0] = true; // keep odor valve closed (blank vial is Onand going to exhaust)
          target_valve_state[1] = true; 
          break;
