function [Trajectories] = SortTrajectories2018(LeverTruncated, MotorTruncated, TrialInfo, ZonesToUse, TargetZones, Params, DoPlot, Handedness)
            ...
            if trialtype == 3
                if ~isempty (cell2mat(Trajectories.(char(trialtag))(Z)))
                    ...
                    elseif myfakezone(cell2mat(Trajectories.TrialIDs.Perturbed(Z))) == 100
                        ...
                        MyTrace = Mean_NoNaNs(all_perturbed_traces);
                        MyShadedErrorBar(1:size(MyTrace,2),MyTrace(1,:),MyTrace(4,:),'k',[],0.5);

                        MyTrace = Mean_NoNaNs(realigned_perturbed_traces);
                        MyShadedErrorBar(1:size(MyTrace,2),MyTrace(1,:),MyTrace(4,:),'r',[],0.5);
                            
                    elseif myfakezone(cell2mat(Trajectories.TrialIDs.Perturbed(Z))) > 20
                        all_perturbed_traces = cell2mat(Trajectories.(char(trialtag))(Z));
                        ...
                        [realigned_perturbed_traces, all_perturbed_traces] = ...
                            ReAlignOffsetPerturbations(all_perturbed_traces,all_hold_times,all_offsets,TargetZones(ZonesToUse(Z),1:3));
