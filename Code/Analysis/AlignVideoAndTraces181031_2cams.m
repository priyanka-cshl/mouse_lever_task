function [handles] = AlignVideoAndTraces181031_2cams(write_video)
...
%% FilePaths
vid_folder = '/Users/Priyanka/Desktop/LABWORK_II/Data/Behavior/Movies/Raw/20181025_cam1_N8';
vid_folder2 = '/Users/Priyanka/Desktop/LABWORK_II/Data/Behavior/Movies/Raw/20181025_cam2_N8';
...
timewindow = [-0.25 (tstop-tstart+1)]; % seconds
...
% display parameters
a = 0.8; % proportion occupied by the video image - choose between 0 and 1
margin = 0.5;
edge = 0.05;
...
set(handles.H2, 'position', [-0.2+margin/2, 0.7, 0.85, 0.25]);
...
colormap(handles.H5, brewermap([24],'rdbu'));
axis off tight
set(handles.H5,'YLim',[0 100]);
set(handles.H5, 'Color', 'none');
...
handles.H4.Position(1) = handles.H1.Position(1) + (handles.H1.Position(3))/2 - zonehalfwidth + 0.005;
handles.H4.Position(2) = handles.H1.Position(2)+handles.H1.Position(4) - 0.001;
...
for i = 1:frames_to_show
    axes(handles.H1);
    frameID = frameID + 1;
    frame = imread(fullfile(vid_folder,[filetag,num2str(frameID),'.png']));
    handles.frame.CData = imresize(frame,0.5);
    
    axes(handles.H1B);
    frame = imread(fullfile(vid_folder2,[filetag2,num2str(frameID),'.png']));
