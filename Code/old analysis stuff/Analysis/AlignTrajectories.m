function [] = AlignTrajectories(LeverTruncated,TrialInfo, ZonesToUse, TargetZones)
    ...
    for Z = 1:numel(ZonesToUse)
        ...
        for M = 2
            for i = 1:3
                H = figure(fig_num+M);
                h = subplot(3,numel(ZonesToUse),Z+numel(ZonesToUse)*(i-1)); hold on
                h.LineWidth = 2;
                h.Box = 'on';
                h.YTick = [];
                % plot target zones
                for j = 1:numel(ZonesToUse)
                    ...
                    fill( [x], [y], ZoneColors(j), 'FaceAlpha', 0.4, 'EdgeColor', 'none');
                end
