...
function [TrajectoriesOut, TrajectoriesIn] = ReAlignOffsetPerturbationsMotor(TrajectoriesIn,MotorTrajectoriesIn,HoldTimes,Offsets,ZoneLimits)
...
for i = 1:size(TrajectoriesIn,1)
    ...
    if ~isempty(f)
        ...
        if perturbationstart_idx < 500
            ...
            TrajectoriesOut(i,:) = [NaN*(ones(1,nans_to_pad)) MotorTrajectoriesIn(i,:) NaN*(ones(1,perturbationstart_idx))];
        else
            points_to_remove = perturbationstart_idx - 500;
            TrajectoriesOut(i,:) = [MotorTrajectoriesIn(i,:) NaN*(ones(1,500))];
        end
    else
        TrajectoriesOut(i,:) = NaN*[MotorTrajectoriesIn(i,:) NaN*(ones(1,500))];
        TrajectoriesIn(i,:) = NaN*MotorTrajectoriesIn(i,:);
    end
