...
int odor_valves[] = {5, 6, 7}; //{4, 5, 6, 7}; // {37, 31, 33, 35} on rig1
int recording_trigger = 4; // this used to be to for odor Vial 0 - not used
...
void setup()
{
  ...
  for (i = 0; i < 4; i++)
  {
    ...
    pinMode(recording_trigger, OUTPUT);
    digitalWrite(recording_trigger, LOW);
  }
      ...
      case 10: // opening/closing handshake
        switch (FSMheader - 10)
        {
          ...
          case 2: // Acquisition stop handshake
            ...
            send_odor_to_manifold();
            camera_on = 1;
            digitalWrite(recording_trigger, LOW); // send sync signal to PCO camera to stop acquiring
            break;
      ...
      case 60: // motor related
        switch (FSMheader - 60)
        {
          ...
          case 2: // move to specific location
            ...
            digitalWrite(visual_pin, (my_location==121));
...
void UpdateOpenLoopParams() // 23.01.2018
{
  ...
  ITIAirState = PassiveTuning_param_array[11];
  if (PassiveTuning_param_array[1] == 800)
  {
    trialstate[0] = 5;
    locationcount = 0;
    pseudosequence_params = num_of_params - 12;
    for (i = 0; i < pseudosequence_params; i++)
    {
      pseudosequence_array[i] = PassiveTuning_param_array[12 + i];
    }
...
void MoveMotor()
{
  ...
  if (!motor_override)// && (trialstate[1] == 4))
  {
    ...
    {
      //I2Cwriter(motor1_i2c_address, 10 + stimulus_state[1]);
      I2Cwriter(motor1_i2c_address, (which_odor!=0)*(10 + stimulus_state[1]));
