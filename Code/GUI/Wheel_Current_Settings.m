function [legend,param,not_ints] = Wheel_Current_Settings(h,caller)

% settings that are sent to the Arduino
legend(1:4) = {'RewardHoldTime' 'RewardDuration' 'MultiReward' 'ITI'};
param(1) = h.current_trial_block.Data(5);
param(2) = h.RewardControls.Data(1);
param(3) = h.MultiRewards.Value;
param(4) = h.current_trial_block.Data(6);

legend(5:7) = {'OdorBuildUp' 'MaxTrialLength' 'TrialOffLag'};
params(5:7) = h.TrialSettings.Data(4:6);

legend(8:10) = {'RewardHoldTime-II' 'RewardDuration-II' 'IRI'};
param(8:10) = h.RewardControls.Data([3 4 2]);
...
%         end
        ...
        if h.which_perturbation.Value == 2 && h.current_trial_block.Data(3) == 1
            param(11:13) = h.PerturbationSettings.Data(2:4);
            % convert to integers for Arduino communication purpose
