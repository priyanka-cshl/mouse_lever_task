...
unsigned int which_transfer_function = 1; // A, B or C
unsigned short transfer_function[99] = {0}; // ArCOM aray needs unsigned shorts
unsigned short transfer_function_A[99] = {0}; // ArCOM aray needs unsigned shorts
unsigned short transfer_function_B[99] = {0}; // ArCOM aray needs unsigned shorts
unsigned short transfer_function_C[99] = {0}; // ArCOM aray needs unsigned shorts
...
void setup()
{
  ...
  for (i = 0; i < num_of_locations; i++)
  {
    ...
    transfer_function_A[i] = 101;
    transfer_function_B[i] = 101;
    transfer_function_C[i] = 101;
  }
...
void loop()
{
  ...
  if (close_loop_mode)
  {
    ...
    switch (which_transfer_function)
    {
      case 1:
        stimulus_state[1] = transfer_function_A[motor_location];
        break;
      case 2:
        stimulus_state[1] = transfer_function_B[motor_location];
        break;
      case 3:
        stimulus_state[1] = transfer_function_C[motor_location];
        break;    
    }
  }
  else
  ...
  if (decouple_reward_and_stimulus)
  {
    ...
    {
      case 1:
        fake_stimulus_state[1] = transfer_function_A[motor_location];
        break;
      case 2:
        fake_stimulus_state[1] = transfer_function_B[motor_location];
        break;
      case 3:
        fake_stimulus_state[1] = transfer_function_C[motor_location];
        break;    
    }
    for (i = 0; i < 2; i++)
    {
  ...
  if (myUSB.available() > 0)
  {
    ...
    switch (round(10 * (FSMheader / 10))) // just a hack to parse out cases into categories
    {
        ...
        else
        {
          switch (FSMheader - 30)
          {
            case 0: // close loop
              ...
              //myUSB.writeUint16(num_of_locations);
              // read all three transfer functions
              myUSB.readUint16Array(transfer_function_A, num_of_locations);
              myUSB.writeUint16Array(transfer_function_A, num_of_locations);
              myUSB.readUint16Array(transfer_function_B, num_of_locations);
              myUSB.writeUint16Array(transfer_function_B, num_of_locations);
              myUSB.readUint16Array(transfer_function_C, num_of_locations);
              myUSB.writeUint16Array(transfer_function_C, num_of_locations);
      ...
      case 40: // toggle MFCs or odor valves
        ...
        break;
      case 50: // update current Target Zone
        serial_clock = millis();
        while ( myUSB.available() < 2 && (millis() - serial_clock) < 1000 )
        { } // wait for serial input or time-out
        if (myUSB.available() < 2)
        ...
        {
          which_transfer_function = myUSB.readUint16();
          target_params[0] = myUSB.readUint16();
          target_params[2] = myUSB.readUint16();
          myUSB.writeUint16(which_transfer_function);
          myUSB.writeUint16(target_params[0]);
          myUSB.writeUint16(target_params[2]);
          myUSB.writeUint16(83);
        }
        break;
